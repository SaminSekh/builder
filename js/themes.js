...
/* Buttons */
button:not(.block-actions *):not(.tb-btn):not(.tab-btn), 
...
div[style*="box-shadow"][style*="border-radius"]:not(.block-actions *):not(.canvas-block > div), 
div[style*="background:#fff"]:not(.block-actions *):not(.canvas-block > div), 
div[style*="background: #fff"]:not(.block-actions *):not(.canvas-block > div), 
div[style*="background:#ffffff"]:not(.block-actions *):not(.canvas-block > div),
div[style*="background: #ffffff"]:not(.block-actions *):not(.canvas-block > div) {
  background: var(--sf-card-bg) !important;
  border-color: var(--sf-border) !important;
}
...
`;
    ...
    function apply(id) {
        ...
        if (theme.type === 'blob' && theme.vars['--sf-blob-1']) {
            css += `
#canvas::before {
  content: '';
  position: absolute;
  ...
  z-index: 0;
  background:
    ${theme.vars['--sf-blob-1'] || ''},
    ${theme.vars['--sf-blob-2'] || ''},
    ${theme.vars['--sf-blob-3'] || ''};
}
`;
        } else {
    ...
    function clear() {
        ...
        if (typeof State !== 'undefined') State.setTheme(null);
  ...
  background:
    ...
    return { getAll, getActive, getActiveId, apply, clear, buildCSSVars, restore };
})();
