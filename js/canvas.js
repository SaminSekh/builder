...
const Canvas = (() => {
    ...
    function showPenTool(target, blockId, path, e = null) {
        ...
        toolbarMoveBtn.onmouseenter = () => {
            ...
            toolbarMoveBtn.style.borderColor = 'rgba(255,255,255,0.22)';
        };
        toolbarMoveBtn.onmouseleave = () => {
            toolbarMoveBtn.style.transform = 'translateY(0)';
            toolbarMoveBtn.style.borderColor = 'rgba(255,255,255,0.08)';
        };
        ...
        dragBtn.onmouseenter = () => {
            ...
            dragBtn.style.borderColor = 'rgba(255,255,255,0.22)';
        };
        dragBtn.onmouseleave = () => {
            dragBtn.style.transform = 'translateY(0)';
            dragBtn.style.borderColor = 'rgba(255,255,255,0.08)';
        };
        ...
        // 2. Up Button
        container.appendChild(createBtn('fa-arrow-up', 'Move Up', 'rgba(255,255,255,0.08)', () => {
            handleAction((bid) => State.moveSubElement(bid, actionPath, 'up'));
            if (isLayoutItemAction) State.setSelectedSubPath(actionPath);
        }));

        // 3. Down Button
        container.appendChild(createBtn('fa-arrow-down', 'Move Down', 'rgba(255,255,255,0.08)', () => {
            handleAction((bid) => State.moveSubElement(bid, actionPath, 'down'));
            if (isLayoutItemAction) State.setSelectedSubPath(actionPath);
        }));

        // 4. Copy Button
        container.appendChild(createBtn('fa-copy', 'Duplicate', 'rgba(255,255,255,0.08)', () => {
            handleAction((bid) => State.duplicateSubElement(bid, actionPath));
        }));

        // 5. Parent Select Button
        container.appendChild(createBtn('fa-level-up-alt', 'Select parent element', 'rgba(255,255,255,0.08)', () => {
        ...
        container.appendChild(createBtn('fa-level-down-alt', 'Select first child element', 'rgba(255,255,255,0.08)', () => {
