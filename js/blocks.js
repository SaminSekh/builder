...
const BlockTypes = {
  ...
  <div class="nav-links-container" data-sf-path="links" style="display:flex;align-items:center;gap:inherit;flex-wrap:wrap;">
    <div class="nav-links" style="display:flex;gap:24px;">${(props.links || []).map((link, i) => `
        <a href="${link.href || '#'}" class="nav-link" data-sf-path="links.${i}" style="color:${props.textColor};text-decoration:none;font-weight:500;transition:opacity .2s;"
...
#canvasFrame.mobile #${uid}.mobile-open .line-3 { transform: translateY(-8px) rotate(-45deg); }
  ...
  carousel: {
    ...
    render(props) {
      const uid = props.customId || ('c' + Math.random().toString(36).substr(2, 9));
      const fit = props.objectFit || 'cover';
      const bgSize = (fit === 'stretch' || fit === 'fill') ? '100% 100%' : fit;
...
(function(){
  window.sfGoTo_${uid}=function(i){
    ...
    dots.forEach(function(d,idx){d.style.background=idx===i?'#fff':'rgba(255,255,255,.5)';});
    window.sf_cur_${uid} = i;
  };
  window.sfNext_${uid}=function(){
    var slides = document.querySelectorAll('.sf-slide-${uid}');
    if(!slides.length) return;
    var nxt = (window.sf_cur_${uid} || 0) + 1;
    if(nxt >= slides.length) nxt = 0;
    window.sfGoTo_${uid}(nxt);
  };
  window.sfPrev_${uid}=function(){
    var slides = document.querySelectorAll('.sf-slide-${uid}');
    if(!slides.length) return;
    var prv = (window.sf_cur_${uid} || 0) - 1;
    if(prv < 0) prv = slides.length - 1;
    window.sfGoTo_${uid}(prv);
  };
  window.sf_cur_${uid} = 0;
  ${props.autoplay ? `
  var iv_${uid} = setInterval(function(){
    var slides = document.querySelectorAll('.sf-slide-${uid}');
    if(!slides.length) { clearInterval(iv_${uid}); return; }
    window.sfNext_${uid}();
  }, ${props.interval || 4000});` : ''}
})();
<\/script>`;
  ...
  video: {
    ...
    render(props) {
      ...
      const autoplayAttr = props.autoplay ? 'autoplay muted playsinline' : '';
      const iframeUrl = props.autoplay ? (embedUrl + (embedUrl.includes('?') ? '&' : '?') + 'autoplay=1&mute=1') : embedUrl;

      const videoElement = isDirect
        ? `<video src="${embedUrl}" controls ${autoplayAttr} style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover;border-radius:12px;"></video>`
        : `<iframe src="${iframeUrl}" title="${props.title}" frameborder="0" allow="accelerometer;autoplay;clipboard-write;encrypted-media;gyroscope;picture-in-picture" allowfullscreen style="position:absolute;top:0;left:0;width:100%;height:100%;"></iframe>`;
  ...
  testimonials: {
    ...
    render(props) {
      ...
      const cards = (props.items || []).map((item, i) => `
        <div data-sf-path="items.${i}" style="background:#fff; color:${props.textColor}; border-radius:12px;padding:28px;box-shadow:0 2px 20px rgba(0,0,0,.07);">
  ...
  applyLayout(props) {
    ...
    return `width:${props.width || '100%'}; max-width:100%; ${heightStyle} margin:${props.margin || '0'}; padding:${props.padding || '0'}; display:${props.display || 'block'}; flex-direction:${props.direction || 'row'}; justify-content:${props.justify || 'center'}; align-items:${props.align || 'center'}; gap:${props.gap || '0'}; flex-wrap:wrap; box-sizing:border-box; ${opacity} ${blur} ${grow} ${shrink} ${basis} ${overflow}`;
  }
